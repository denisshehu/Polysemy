...
def plot_2d(x, y, x_label, y_label, figure_path, n_senses=None):
    ...
    if n_senses is None:
        ...
        plt.scatter([x[i] for i in monosemes], [y[i] for i in monosemes], c='black', label='Monoseme')
        plt.scatter([x[i] for i in polysemes], [y[i] for i in polysemes], c='red', label='Polyseme')
        plt.legend()
