...
def _calculate(query, query_annuli):
    ...
    for annulus, s, r in query_annuli:
        if len(annulus) != 0:
            diagram1 = compute_persistence_diagram(annulus, dimension)

            euclidean_annulus = sample_from_annulus(len(annulus), dimension, s, r, seed=1,
                                                    ambient_dimension=np.shape(query.point)[0])
            diagram2 = compute_persistence_diagram(euclidean_annulus, dimension)
...
def compute_persistence_diagram(points, dimension):
    persistence_diagrams = ripser.ripser(X=points, maxdim=(dimension - 1))['dgms']
    filtered_persistence_diagrams = filter_persistence_diagrams_dynamic(persistence_diagrams)
