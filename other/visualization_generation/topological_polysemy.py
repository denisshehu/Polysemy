...
def visualize(point_cloud, filename_prefix):
    ...
    x_monosemes = [query.topological_polysemy for query in monosemes]
    y_monosemes = [query.n_senses for query in monosemes]

    x_polysemes = [query.topological_polysemy for query in polysemes]
    ...
    plt.xlabel('Topological polysemy')
    plt.ylabel('Number of senses')
    filename_prefix = f'{filename_prefix}_' if filename_prefix is not None else ''
    figure_path = os.path.join(results_directory, f'{filename_prefix}topological_polysemy.png')
...
def plot3d(point_cloud, filename_prefix):
    ...
    c = [query.topological_polysemy for query in queries]

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    scatter = ax.scatter(x, y, z, c=c, cmap='plasma')
    fig.colorbar(scatter, label='Topological polysemy')
