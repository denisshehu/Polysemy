...
def visualize(point_cloud, filename_prefix):
    ...
    x_monosemes = [query.euclidicity for query in monosemes]
    y_monosemes = [query.n_senses for query in monosemes]

    x_polysemes = [query.euclidicity for query in polysemes]
    ...
    plt.xlabel('Euclidicity')
    plt.ylabel('Number of senses')
    filename_prefix = f'{filename_prefix}_' if filename_prefix is not None else ''
    figure_path = os.path.join(results_directory, f'{filename_prefix}euclidicity.png')
...
def plot3d(point_cloud, filename_prefix):
    ...
    c = [query.euclidicity for query in queries]

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    scatter = ax.scatter(x, y, z, c=c, cmap='plasma')
    fig.colorbar(scatter, label='Euclidicity')
